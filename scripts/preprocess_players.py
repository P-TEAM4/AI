...
def process_player_rows(match_json: Dict[str, Any],
    ...
    # 팀별 합계 데미지/골드 계산 → share 계산용 (참고용으로 남김)
    team_totals = {
        100: {"dmg": 0, "gold": 0},
        200: {"dmg": 0, "gold": 0},
    }
    ...
    for p in participants:
        ...
        row = {
            ...
            # ===== 효율성 지표 (승패 독립적) =====
            "goldPerMinute": challenges.get("goldPerMinute", gold / max(1, game_duration_min)),
            "damagePerMinute": challenges.get("damagePerMinute", dmg / max(1, game_duration_min)),
            "visionScorePerMinute": challenges.get("visionScorePerMinute", p.get("visionScore", 0) / max(1, game_duration_min)),
            ...
            "soloTurretsLategame": challenges.get("soloTurretsLategame", 0),
            ...
            # ===== 생존력 지표 =====
            "deathsByEnemyChamps": challenges.get("deathsByEnemyChamps", deaths),
            ...
            # ===== 기존 share 지표 (참고용으로 남김, 하지만 모델에서는 사용하지 않을 예정) =====
            "dmg_share": dmg / team_totals[team_id]["dmg"] if team_totals[team_id]["dmg"] > 0 else 0.0,
            "gold_share": gold / team_totals[team_id]["gold"] if team_totals[team_id]["gold"] > 0 else 0.0,
        }
